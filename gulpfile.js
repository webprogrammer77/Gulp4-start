    ...
    if (spritePngIn + "*.png" !== false) {
      ...
      let fileName =
        ...
        Math.random()
          ...
          .replace(/[^0-9]/g, "") +
        ".png";
      let spriteData = gulp.src(spritePngIn + "*.png").pipe(
        spritesmith({
          imgName: fileName,
      ...
      let cssStream = spriteData.css.pipe(gulp.dest(spritePngOut));
...
function fontscopy() {
  ...
  return gulp.src(fontsIn).pipe(gulp.dest(fontsOut));
...
function deploy() {
  ...
  return gulp
    ...
    .pipe(conn.dest("/prohorov-igor.ru/public_html/"));
}
...
gulp.task("sprite-svg", gulp.series(["sprite-made-svg", "sprite-svg-copy"]));
gulp.task("sprite", gulp.series(["sprite-svg", "sprite-png"]));
gulp.task("convert", gulp.series(towoffTwo, towoff));
